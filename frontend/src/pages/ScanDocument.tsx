...
const MODE_LABELS: { mode: ScanMode; label: string; desc: string }[] = [
  {
    mode: "bw",
    label: "Hitam-Putih",
    desc: "Seperti hasil scan â€” kontras tinggi",
  },
  {
    mode: "grayscale",
    label: "Grayscale",
    desc: "Bayangan halus tetap terlihat",
  },
  {
    mode: "color",
    label: "Warna Enhanced",
    desc: "Warna asli + noda kopi direduksi",
  },
];
...
export default function ScanDocument() {
    ...
    try {
      ...
      showToast(
        res.data.doc_detected
          ? `Dokumen terdeteksi & diluruskan âœ“ â€” ${res.data.output_resolution}`
          : "Tepi dokumen tidak terdeteksi â€” frame penuh diproses.",
  ...
  const handleDownload = (format: "png" | "jpg") => {
    ...
    a.click();
    showToast(`Download ${format.toUpperCase()} selesai âœ“`);
  };
    ...
    >
      ...
      <div className="border-b border-border pb-4">
        ...
        <p className="text-sm text-muted ml-12">
          Foto kertas / dokumen â†’ otomatis dideteksi, diluruskan perspektif,
          dihilangkan bayangan & noda, diperjelas teksnya. Pipeline 8 tahap
          OpenCV â€” seperti CamScanner.
        </p>
      </div>
              ...
              <>
                ...
                <div>
                  ...
                  <p className="text-xs text-muted mt-1">
                    Foto kertas, KTP, KK, surat, nota â€” JPG / PNG / WebP
                  </p>
                </div>
              </>
            )}
          ...
          {result && (
            <div className="grid grid-cols-3 gap-2">
              {MODE_LABELS.map(({ mode, label, desc }) => (
                ...
                >
                  <p
                    className={`text-xs font-semibold ${activeMode === mode ? "text-accent" : "text-white"}`}
                  >
                    {label}
                  </p>
            ...
            <div className="space-y-1.5">
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-xs font-medium text-accent uppercase tracking-widest">
                  Sesudah{" "}
                  {result
                    ? `â€” ${MODE_LABELS.find((m) => m.mode === activeMode)?.label}`
                    : ""}
                </span>
              </div>
            ...
            <div className="flex items-center gap-3 flex-wrap text-xs text-muted px-1">
              ...
              {result.doc_detected ? (
                <span className="text-green-400 flex items-center gap-1">
                  <Check size={11} /> Dokumen terdeteksi & diluruskan
                </span>
              ) : (
                <span className="text-yellow-400 flex items-center gap-1">
                  <AlertCircle size={11} /> Tepi tidak terdeteksi â€” frame
                  penuh
                </span>
              )}
            </div>
          ...
          {result && (
            <div className="rounded-xl border border-border bg-card p-4">
              <p className="text-sm font-semibold text-white mb-3">
                Distribusi Intensitas â€” Before vs After
              </p>
              <HistogramChart
          ...
          {!result && !isProcessing && (
            <div className="rounded-xl border border-border bg-card p-5 space-y-3">
              <p className="text-sm font-semibold text-white">
                ðŸ’¡ Tips Foto Terbaik
              </p>
              <ul className="space-y-2 text-xs text-muted">
                <li className="flex items-start gap-2">
                  <span className="text-accent flex-shrink-0 mt-0.5">âœ“</span>
                  Foto di bawah cahaya cukup (jangan terlalu gelap/silau)
                </li>
                <li className="flex items-start gap-2">
                  <span className="text-accent flex-shrink-0 mt-0.5">âœ“</span>
                  Letakkan dokumen di atas permukaan kontras (mis: meja gelap
                  untuk kertas putih)
                </li>
                <li className="flex items-start gap-2">
                  <span className="text-accent flex-shrink-0 mt-0.5">âœ“</span>
                ...
                <li className="flex items-start gap-2">
                  ...
                  Teks yang tertutup noda tinta gelap total tidak bisa
                  dipulihkan
                </li>
              </ul>
            </div>
